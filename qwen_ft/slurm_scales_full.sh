...
# Explicit per-group LMO scales over the full 8-point LR grid in one array,
# for alpha-sweep series added after the grid was extended.
OPTIM=$1; TAG=$2; ALPHA=$3; SCALES=$4; MODEL=$5
...
python qwen_ft/finetune.py --optim $OPTIM --lr $LR \
  --alpha $ALPHA --scales "$SCALES" --model $MODEL --tag $TAG
